...
if page == "🏠 Dashboard":
    ...
    df_payment_received["Date"] = pd.to_datetime(df_payment_received["Date"], errors="coerce")
    df_investment["Date"] = pd.to_datetime(df_investment["Date"], errors="coerce")

    df_cow_log = df_cow_log[df_cow_log["Date"] >= START_DATE]
    df_expense = df_expense[df_expense["Date"] >= START_DATE]
    df_milk_m = df_milk_m[df_milk_m["Date"] >= START_DATE]
    df_milk_e = df_milk_e[df_milk_e["Date"] >= START_DATE]
    df_payment_received = df_payment_received[df_payment_received["Date"] >= START_DATE]
    df_investment = df_investment[df_investment["Date"] >= START_DATE]
